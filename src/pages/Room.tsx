...
import { ArrowLeft, Copy, Mic, MicOff, Send, Users, Youtube, Upload, Reply, Crown, X, FolderOpen, UploadCloud, Smile, Image as ImageIcon, CheckCheck, SendHorizonal } from 'lucide-react';
...
export default function Room() {
    ...
    const handleSendChat = (e?: React.FormEvent, customText?: string, imageUrl?: string, isSticker?: boolean) => {
                    ...
                    <div className="flex-1 relative flex flex-col overflow-hidden w-full bg-[#090d14] md:bg-transparent">
                        {chatWallpaper && (
                            <div className="absolute inset-0 z-0 pointer-events-none">
                                <img src={chatWallpaper} style={{ opacity: wallpaperOpacity }} className="w-full h-full object-cover object-center transition-opacity" alt="chat-wallpaper" />
                            </div>
                        )}
                        ...
                        <div className="p-3 space-y-1 md:space-y-2 relative z-10 min-h-full">
                            {messages.map(msg => (
                                <div key={msg.id} className={cn("flex gap-1.5 mb-2", msg.sender === profile?.name ? "justify-end" : "justify-start")}>
                                    {msg.sender !== profile?.name && (
                                        msg.avatar ? (
                                            <div className="w-6 h-6 rounded-full p-[1px] shrink-0 mt-0.5 flex-none" style={{ background: msg.frameColor || '#333' }}>
                                            ...
                                            <div className="w-6 h-6 rounded-full shrink-0 flex-none bg-[#1e293b] flex items-center justify-center border border-white/5 mt-0.5">
                                                <span className="text-[10px] font-bold text-slate-300">{msg.sender.charAt(0).toUpperCase()}</span>
                                            </div>
                                        )
                                    )}
                                    <div className="flex flex-col min-w-0 max-w-[85%]">
                                        <div className={cn(
                                            ...
                                            (msg.isSticker && !msg.text) ? "bg-transparent p-0" : cn("backdrop-blur-sm shadow-sm", msg.sender === profile?.name ? "bg-[#334155]/90 border border-slate-600/50 rounded-xl rounded-tr-[2px]" : "bg-[#1e293b]/90 border border-slate-600/50 rounded-xl rounded-tl-[2px]"),
                                            (!msg.text && msg.imageUrl && !msg.isSticker) ? "p-1" : ((msg.isSticker && !msg.text) ? "" : "py-1.5 px-2.5")
                                        )}>
                                            {(!msg.isSticker || msg.text) && msg.sender !== profile?.name && (
                                                <div className="flex items-baseline gap-3 mb-0.5 px-0.5">
                                                    <span className="text-[11px] font-bold text-slate-300 truncate leading-none" style={{ color: msg.frameColor || '#cbd5e1' }}>{msg.sender}</span>
                                                </div>
                                            )}
                                            {msg.replyTo && (
                                                <div className={cn("mb-1 pl-1.5 border-l-2 border-cyan-500/50 bg-black/20 py-0.5 px-1.5 flex justify-between gap-1 items-center", (msg.isSticker && !msg.text) ? "rounded-lg" : "rounded-sm")}>
                                                    <div className="flex flex-col truncate flex-1 min-w-0">
                                                        <span className="text-[10px] font-bold text-cyan-400 leading-tight">{msg.replyTo.sender}</span>
                                                        <span className="text-[10px] text-slate-300 truncate h-3 overflow-hidden leading-tight">{msg.replyTo.text}</span>
                                                    </div>
                                                    {msg.replyTo.imageUrl && (
                                                        <img src={msg.replyTo.imageUrl} className="w-6 h-6 object-cover rounded-[3px] opacity-80 shrink-0" alt="reply-thumb" />
                                                    )}
                                                </div>
                                            )}
                                            <div className="flex flex-col w-full max-w-full">
                                                {msg.imageUrl && (
                                                    <img src={msg.imageUrl} className={cn("object-contain", (msg.isSticker && !msg.text) ? "max-w-[120px] md:max-w-[140px] drop-shadow-lg" : "rounded-lg max-w-[140px] md:max-w-[180px] border border-white/5")} alt={msg.isSticker ? "sticker" : "photo"} />
                                                )}
                                                {msg.text && (
                                                    <p className="text-[13px] text-slate-100 break-words leading-[1.3] pt-0.5 whitespace-pre-wrap">{msg.text}</p>
                                                )}
                                            </div>
                                            <div className={cn("flex items-center justify-end gap-1 mt-0.5", (msg.isSticker && !msg.text) ? "absolute bottom-0 right-0 translate-x-1/4 translate-y-1/4 drop-shadow-md bg-black/50 backdrop-blur-sm rounded-full px-1.5 py-[2px] w-fit z-10" : "px-0.5")}>
                                                <span className="text-[9px] text-slate-400/80 font-mono leading-none">{msg.time}</span>
                                                {msg.sender === profile?.name && <CheckCheck className="w-3 h-3 text-cyan-500" />}
                                            </div>
                                            <button 
                                                onClick={() => setReplyingTo({ id: msg.id, sender: msg.sender, text: msg.isSticker ? 'Sticker' : (msg.text || 'Photo'), imageUrl: msg.imageUrl })}
                                                className={cn("absolute top-1/2 -translate-y-1/2 opacity-30 sm:opacity-0 sm:group-hover:opacity-100 transition-all p-1 hover:bg-white/10 rounded-full active:scale-95 z-20 bg-black/40 backdrop-blur-md outline-none", msg.sender === profile?.name ? "-left-8" : "-right-8", (msg.isSticker && !msg.text) && (msg.sender === profile?.name ? "-left-10" : "-right-10"))}
                                                title="Reply"
                                            >
                                ...
                                <div className="h-10 bg-[#0f1521] flex items-center px-2 gap-3 overflow-x-auto shrink-0 border-t border-slate-800">
                                    ...
                                    <label className="p-1 text-cyan-400 hover:text-cyan-300 cursor-pointer flex items-center shrink-0">
                                        <div className="p-1 bg-cyan-500/10 rounded flex items-center justify-center">
                                            <span className="text-[10px] font-bold">+ Tambah Paket Baru</span>
                                        </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in zoom-in-95 duration-200">
                    <div className="bg-[#0f1521] border border-[#2e3c5a] rounded-2xl w-full max-w-sm overflow-hidden flex flex-col shadow-2xl relative">
                        ...
                        <div className="relative w-full h-[55dvh] bg-[#090d14] overflow-hidden">
                            <img src={previewWallpaper} style={{ opacity: previewOpacity }} className="absolute inset-0 w-full h-full object-cover object-center transition-opacity" alt="preview" />
                        </div>
