...
import { ArrowLeft, Copy, Mic, MicOff, Send, Users, Youtube, Upload, Reply, Crown, X, FolderOpen, UploadCloud, Smile, Image as ImageIcon } from 'lucide-react';
...
export default function Room() {
    ...
    useEffect(() => {
        ...
        if (saved) {
            try { setCustomStickers(JSON.parse(saved)); } catch (e) {}
        }
    }, []);
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            Object.values(speakingTimeoutsRef.current).forEach(t => clearTimeout(t));
        };
    }, [socketRef]);
    ...
    const handleSendChat = (e?: React.FormEvent, customText?: string, imageUrl?: string) => {
        ...
        if (chatInputRef.current && !customText) chatInputRef.current.value = '';
        setReplyingTo(null);
    };
        ...
        <div className="flex flex-col h-[100dvh] bg-[#0A0F1A] overflow-hidden md:p-3">
                ...
                <div className="w-full aspect-video bg-black shrink-0 relative z-10 shadow-[0_10px_30px_rgba(0,0,0,0.5)] md:shadow-none">
                    {videoUrl ? (
                        <div className="w-full h-full pointer-events-auto transition-opacity duration-1000 opacity-100 flex items-center justify-center">
                            ...
                            {!isHost && (
                                <div className="absolute inset-0 z-20 cursor-default" onClick={(e) => { e.stopPropagation(); e.preventDefault(); }} />
                            )}
                        </div>
                    ) : (
                                            ...
                                            <div className="relative avatar-wrapper" style={{ '--color': baseColor } as any}>
                                                ...
                                                >
                                                    {p.avatar ? (
                                                        ...
                                                        <span className="text-xl font-bold" style={{ color: baseColor }}>{p.name.charAt(0).toUpperCase()}</span>
                                                    )}
                                                </div>
            ...
            <div className="md:col-span-4 flex flex-col flex-1 overflow-hidden bg-transparent pb-4 md:pb-0 z-20">
                ...
                <div className="flex-1 overflow-hidden relative flex flex-col md:bg-white/5 md:backdrop-blur-xl md:border md:border-white/10 md:rounded-[24px]">
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-transparent">
                        {messages.map(msg => (
                            <div key={msg.id} className="flex gap-2 mb-3">
                                {msg.avatar ? (
                                    <div className="w-8 h-8 rounded-full p-[2px] shrink-0 mt-0.5 flex-none" style={{ background: msg.frameColor || '#333' }}>
                                        <img src={msg.avatar} alt={msg.sender} className="w-full h-full rounded-full object-cover bg-black/50" />
                                    </div>
                                ) : (
                                    <div className="w-8 h-8 rounded-full p-[2px] shrink-0 mt-0.5 flex-none" style={{ background: msg.frameColor || 'rgba(6,182,212,0.5)' }}>
                                        <div className="w-full h-full rounded-full bg-[#0a0f1a] flex items-center justify-center">
                                            <span className="text-[12px] font-bold" style={{ color: msg.frameColor || '#22d3ee' }}>{msg.sender.charAt(0)}</span>
                                        </div>
                                    </div>
                                )}
                                <div className="flex flex-col flex-1 min-w-0">
                                    <div className="bg-white/5 border border-cyan-500/20 rounded-2xl rounded-tl-sm py-1.5 px-3 w-fit max-w-[90%] shadow-[0_4px_15px_rgba(0,0,0,0.2)] relative group backdrop-blur-md min-w-[80px]">
                                        <div className="flex justify-between items-baseline gap-3 mb-0.5">
                                            <span className="text-[11px] font-bold text-slate-300 truncate leading-none">{msg.sender}</span>
                                        </div>
                                        {msg.replyTo && (
                                            <div className="mb-1.5 pl-2.5 border-l-2 border-cyan-500/50 bg-black/20 py-1 px-2 rounded-r-lg">
                                                <p className="text-[10px] font-bold text-cyan-400 mb-0.5 leading-none">{msg.replyTo.sender}</p>
                                                <p className="text-[11px] text-slate-400 truncate line-clamp-1 h-3 overflow-hidden leading-none">{msg.replyTo.text}</p>
                                            </div>
                                        )}
                                        <div className="flex items-end gap-2 justify-between mt-1">
                                            <div className="flex flex-col gap-1 w-full max-w-full">
                                                {msg.imageUrl && (
                                                    <img src={msg.imageUrl} className="rounded-lg max-w-full md:max-w-[200px] object-contain border border-white/10" alt="stiker" />
                                                )}
                                                {msg.text && (
                                                    <p className="text-[13px] text-slate-200 break-words leading-snug">{msg.text}</p>
                                                )}
                                            </div>
                                            <span className="text-[9px] text-slate-500 font-mono shrink-0 ml-1 mb-0.5">{msg.time}</span>
                                        </div>
                                        <button 
                    ...
                    <div className="bg-[#0A0F1A] flex flex-col shrink-0">
                        {replyingTo && (
                            <div className="px-4 py-2 flex items-center justify-between bg-[#1e293b] rounded-t-2xl mb-1 mx-2 mt-2">
                                <div className="flex flex-col flex-1 min-w-0 pr-2 border-l-4 border-cyan-500 pl-2">
                                    <span className="text-[11px] text-cyan-400 font-bold mb-0.5">Replying to {replyingTo.sender}</span>
                                    <span className="text-[13px] text-slate-300 truncate">{replyingTo.text}</span>
                                </div>
                                <button onClick={() => setReplyingTo(null)} className="p-2 hover:bg-white/10 rounded-full text-slate-400 hover:text-white transition-colors">
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                        )}
                        ...
                        {showStickers && (
                            <div className="h-48 bg-[#1e293b] border-t border-slate-700/50 p-3 overflow-y-auto">
                                <div className="flex items-center justify-between mb-3 text-sm font-bold text-slate-300 px-1">
                                    <span>Custom Stickers</span>
                                    <label className="text-cyan-400 cursor-pointer text-xs font-normal bg-cyan-500/10 px-2 py-1 rounded hover:bg-cyan-500/20 transition-colors">
                                        + Buat Sticker
                                        <input type="file" accept="image/*" className="hidden" onChange={handleCreateSticker} />
                                    </label>
                                </div>
                                {customStickers.length === 0 ? (
                                    <div className="text-center text-slate-500 text-[11px] mt-10">Belum ada custom sticker</div>
                                ) : (
                                    <div className="grid grid-cols-4 gap-2">
                                        {customStickers.map((sticker, idx) => (
                                            <div key={idx} className="relative group aspect-square bg-[#0a0f1a] rounded-lg overflow-hidden border border-slate-700">
                                                <img 
                                                    src={sticker} 
                                                    alt={`sticker-${idx}`}
                                                    className="w-full h-full object-cover cursor-pointer hover:opacity-80 transition-opacity"
                                                    onClick={() => {
                                                        handleSendChat(undefined, '', sticker);
                        ...
                        <form onSubmit={handleSendChat} className="p-2 w-full flex items-end gap-2 bg-[#0A0F1A]">
                            <div className="flex-1 bg-[#1e293b]/70 backdrop-blur-md border border-slate-700/50 rounded-3xl flex items-center pr-2 pl-1 relative z-10 transition-colors focus-within:border-cyan-500/30">
                                <button type="button" onClick={() => setShowStickers(!showStickers)} className={cn("p-2 text-slate-400 hover:text-white shrink-0 active:scale-95 transition-all", showStickers && "text-cyan-400")}>
                                ...
                                <input 
                                    ...
                                    ref={chatInputRef}
                                    placeholder="Message..."
                                    className="flex-1 bg-transparent border-none px-2 py-3 text-[14px] focus:outline-none text-slate-200 placeholder:text-slate-400 w-full min-w-0"
                            ...
                            <button 
                                type="submit"
                                className="w-12 h-12 rounded-full border flex items-center justify-center transition-colors disabled:opacity-50 border-cyan-500/30 bg-cyan-500 text-slate-900 hover:bg-cyan-400 shrink-0 shadow-[0_0_15px_rgba(6,182,212,0.2)] mb-0.5"
                            >
                                <Send className="w-[22px] h-[22px] ml-0.5 translate-x-[1px]" />
                            </button>
                        </form>
                    </div>
                </div>
